...
/// https://github.com/Rahiche/soft_edge_blur/blob/main/lib/src/soft_edge_blur.dart
...
class SoftEdgeBlur extends StatelessWidget {
  ...
  SoftEdgeBlur({super.key, required this.child, required this.edges}) {
    assert(
  ...
  Rect _getEdgeRect(EdgeBlur edge, Size size, double devicePixelRatio) {
    ...
    switch (edge.type) {
      case EdgeType.topEdge:
        return Rect.fromLTRB(0, 0, width, edge.size * devicePixelRatio);
      ...
      case EdgeType.leftEdge:
        return Rect.fromLTRB(0, 0, edge.size * devicePixelRatio, height);
